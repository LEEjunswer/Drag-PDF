                ...
                if (barcode != null && barcode.isNotEmpty) {
                  showDialog<String>(
                    ...
                    builder: (BuildContext context) => AlertDialog(
                      title: Text(Localization.of(context).string('choose_an_option')),
                      content: Text(Localization.of(context).string('content_home_screen_dialog')),
                      actions: [
                        TextButton(
                          onPressed: () {
                            context.pop();
                            FileDialog.add(
                              ...
                              await loadFilesOrImages(LoaderOf.imagesFromGallery, barcode),
                              loadFileFromFileSystem: () async =>
                              await loadFilesOrImages(LoaderOf.filesFromFileSystem, barcode),
                            );
                          },
                          child: Text(Localization.of(context).string('가져오기')), // LOAD
                        ),
                        TextButton(
                          onPressed: () async => await scanImages(barcode),
                          child: Text(Localization.of(context).string('스캔')), // SCAN
                        ),
                        TextButton(
                          onPressed: () => context.pop('취소'),
                          child: Text(
                            Localization.of(context).string('취소'), // Cancel
